...
- PROFILE-FIRST CLASSIFICATION (LOCKED): Before deciding Income or Expense, you MUST check the Financial Profile (sections 2-3, 8-11). FIRST check if the transaction is a TRANSFER: compare the relatedParty/merchant against Bank Account names (section 3) and Cash Account names (section 2) — if the money is moving BETWEEN the user's own accounts (e.g. "transfer dari Maybank ke CIMB", "pindah duit ke peti besi"), set transactionType to "TRANSFER", NOT Income or Expense. SECOND check if it involves the user's own Businesses (section 9) — money moving between businesses or between owner and business is also a TRANSFER or OWNER_TRANSACTION. THIRD: if not a transfer, determine direction: (a) Incoming money (customer paying, loan disbursement, capital injection) = INCOME or OWNER_TRANSACTION(CAPITAL_INJECTION); (b) Outgoing money (paying supplier, loan repayment, owner drawing) = EXPENSE or OWNER_TRANSACTION(DRAWING) or PAYABLE or DEBT. Never default to EXPENSE just because money left an account — it may be an internal transfer or owner transaction.
