  ...
  function generateFallbackAssistantResponse(query: string, financialContext: any) {
    ...
    if (q.includes("aws") || q.includes("saas") || q.includes("software")) {
      ...
      if (match) {
        ...
        text = `### 🔍 AI Q&A Discovery Match: AWS & SaaS Expenses\n\nI have retrieved the transaction matching **"${match.partyName}"** under the **"${match.categoryName}"** category.\n\n- **Amount:** RM ${match.amountMyr.toFixed(2)}\n- **Date:** ${match.date}\n- **Explanation:** Active SaaS subscriptions are properly classified as general operating disbursements. Regular audit cycles confirm alignment with your workspace ledger thresholds.`;
      } else {
        text = `### 🔍 AI Search: No explicit SaaS / AWS matches\n\nNo records matching "AWS" or "SaaS" were discovered in the active workspace transaction ledger. \n\nWe recommend confirming if your utilities are classified correctly. You can register a new classification in the **AI Learning Layer Registry** to pre-classify future tech subscriptions.`;
      }
    } else if (q.includes("health") || q.includes("diagnostic") || q.includes("analysis")) {
      text = `### 🩺 MYKERANI Automated Workspace Health Diagnostic\n\nI have compiled a cognitive audit of the active ledger workspace dataset:\n\n1. **Liquid Capital Reserves:** RM ${reserves.toLocaleString("en-MY", { minimumFractionDigits: 2 })} (comprising RM ${totalCash.toLocaleString("en-MY", { minimumFractionDigits: 2 })} in cash floats and RM ${totalBank.toLocaleString("en-MY", { minimumFractionDigits: 2 })} in bank vaults).\n2. **Operating Inflows vs Outlays:** Total revenue currently stands at **RM ${totalIncome.toLocaleString("en-MY", { minimumFractionDigits: 2 })}**, with operating expenses of **RM ${totalExpense.toLocaleString("en-MY", { minimumFractionDigits: 2 })}**.\n3. **Liquidity Buffer Runway:** Based on a rolling daily expenditure calculation, your liquid capital offers an estimated **${computedRunway} days** of continuous cashflow buffer.\n\n**Strategic Advisor Recommendations:**\n- Verify outstanding collections from your receivables (RM ${(incomes.filter((i: any) => i.type === "RECEIVABLE").reduce((acc: number, curr: any) => acc + curr.amountMyr, 0)).toLocaleString("en-MY", { minimumFractionDigits: 2 })} outstanding).\n- Register vendor category patterns to automate billing validation loops.`;
    } else if (q.includes("forecast") || q.includes("runway") || q.includes("days")) {
      text = `### 📊 Liquidity buffer and Cashflow Forecast Advisory\n\nCurrent financial data indicates an operating Runway of approximately **${computedRunway} days**.\n\n- **Reserves:** RM ${reserves.toLocaleString("en-MY", { minimumFractionDigits: 2 })}\n- **Upcoming Commitments:** ${commitments.length} active recurring billing profiles mapped.\n- **Analysis:** Your liquidity position is **${healthStatus}**. We advise initiating invoicing callbacks on overdue receivables at least 10 days before primary payables clearance cycles to preserve optimal liquid reserves.`;
    } else if (q.includes("evidence") || q.includes("receipt") || q.includes("maybank") || q.includes("statement")) {
      const match = evidence[0];
      if (match) {
        linkedEvidenceIds.push(match.id);
        if (match.linkedRecordId) linkedRecordIds.push(match.linkedRecordId);
        text = `### 📂 Cognitive Evidence Retrieval\n\nI discovered **1 matching audit attachment** inside the financial evidence vault:\n\n- **File Name:** \`${match.fileName}\`\n- **Document Type:** \`${match.documentType}\`\n- **Audit Status:** Linked to financial record reference \`${match.linkedRecordId ? match.linkedRecordId.substring(0,8) : "None"}\`\n\nThis package has been audited and compiled under organizational boundary safety standards. Let me know if you would like to view the connected txn metrics.`;
      } else {
        text = `### 📂 Cognitive Evidence Discovery: Empty Vault\n\nNo uploaded files or audit attachments were discovered inside the active workspace evidence index.\n\nYou can upload receipt images or bank statements inside the **AI OCR Scanner** or **Financial Evidence Package** tabs. The AI will learn and pre-classify these files automatically.`;
      }
    } else {
      text = `### 🤖 Welcome to MYKERANI AI Copilot Desk\n\nHello! I am your cognitive financial analyst. I can answer complex questions about your active ledger and retrieve documents instantly.\n\n- **Liquid Reserves:** RM ${reserves.toLocaleString("en-MY", { minimumFractionDigits: 2 })}\n- **Transactions Analyzed:** ${events.length} records\n- **Pre-classified memory:** ${patterns.length} vendor profiles\n\n**Suggested Prompts:**\n- Try typing: *"Run workspace financial health diagnostic"* \n- Try typing: *"Find AWS or SaaS-related expenses"* \n- Try typing: *"Retrieve receipt audit evidence"*`;
    }
